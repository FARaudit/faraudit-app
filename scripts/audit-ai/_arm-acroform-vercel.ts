// ARM — AUDIT_INGEST_ACROFORM_FIELDS = true on VERCEL PRODUCTION (Rule 17 parity with the audit-worker).
// CEO authorized in words 2026-08-03 ("execute your recommendation"), after PR #404 merged as d12ca39b and the
// flag was armed + execution-proven on audit-worker.
//
// WHY PARITY IS REQUIRED HERE, not merely tidy: `extractText` is reached by THREE Vercel routes —
// src/app/api/audit/route.ts, src/app/api/audit/resolve/route.ts, src/app/api/telegram/route.ts. With the flag
// set on the worker only, the SAME document ingests two different ways depending on how it was submitted.
//
...
//   npx dotenv -e .env.local -- npx tsx scripts/audit-ai/_arm-displaced-run-vercel.ts
import dotenv from "dotenv";
dotenv.config({ path: ".env.local", quiet: true });
const TOKEN = process.env.VERCEL_TOKEN!;
const PROJ = "prj_oqyqfwO0qJmkSAO9Hvt7VxbLUToD";
const TEAM = "team_4FAowTLgslDBY6aZ0acPaES0";
const KEY = "AUDIT_INGEST_ACROFORM_FIELDS";
