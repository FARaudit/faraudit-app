// ARM — AUDIT_INGEST_DENOMINATOR_RECONCILE = true on VERCEL PRODUCTION (Rule 17 parity with the audit-worker).
// CEO authorized in words 2026-08-02 ("arm it"), after PR #395 merged as 6ba4f30.
//
// POST, then READ BACK from the live list and let the read-back be the sole authority — a POST outcome is
// advisory (Vercel signals already-exists as HTTP 400 + ENV_CONFLICT, not 409). Does NOT redeploy: a running
// build carries the OLD env snapshot, so a fresh build is a separate explicit step.
//   npx dotenv -e .env.local -- npx tsx scripts/audit-ai/_arm-denominator-vercel.ts
import dotenv from "dotenv";
dotenv.config({ path: ".env.local", quiet: true });
const TOKEN = process.env.VERCEL_TOKEN!;
const PROJ = "prj_oqyqfwO0qJmkSAO9Hvt7VxbLUToD";
const TEAM = "team_4FAowTLgslDBY6aZ0acPaES0";
const KEY = "AUDIT_INGEST_DENOMINATOR_RECONCILE";
