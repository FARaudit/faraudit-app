...
(async () => {
  ...
  for (const c of cands.slice(0, 12)) {
    ...
    // COMPETITIVE screen (card 219): reject sole-source / brand-name-ONLY designations — we want a doc that
    // exercises the committal BID path. "brand name or equal" is COMPETITIVE (allowed); "sole source" is not.
    const soleSource = /\bsole\s*source\b|single\s+source|only\s+one\s+(?:responsible\s+)?source|intent\s+to\s+(?:sole|award\s+without)|brand\s*name\s*only|no\s+substitut/i.test(text);
    const part12 = hasSF1449 && has2121 && !soleSource;
    gated.push(`  ${part12 ? "✅" : soleSource ? "🚫sole-src" : "⬜"} ${c.sol || c.noticeId} · NAICS ${c.naics} · ${bytes}B · SF1449=${hasSF1449} 212-1=${has2121} NMR=${hasNmr} · ${c.title}`);
    if (part12 && bytes > 4000 && !picked) picked = { c, text, hasNmr };
    if (part12 && bytes > 4000 && hasNmr) { picked = { c, text, hasNmr }; break; } // prefer a competitive NMR doc
    await sleep(300);
  }
  ...
  const outPath = `scripts/audit-ai/gold-sets/${label}-card219-FULL-SOURCE.txt`;
