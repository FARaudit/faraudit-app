// ARM — AUDIT_ZERO_ATTESTATION_INCOMPLETE = true on VERCEL PRODUCTION (Rule 17 parity with audit-worker).
// CEO authorized in words 2026-08-05: "merge #478 and arm AUDIT_ZERO_ATTESTATION_INCOMPLETE".
//
// RUN THIS **BEFORE** MERGING #478. Vercel snapshots env at BUILD START, so a flag added after the merge
// build begins is absent from the very build that carries the code (measured 2026-08-05: 22s late needed a
// `vercel redeploy` to take, while every read-back said "armed").
//
// WHY PARITY IS REQUIRED: gateV2Outcome is reached by src/lib code that Vercel routes exercise, so armed on
// the worker only, the SAME unattested package would return INCOMPLETE from one entry point and a committal
// "Coverage complete (100%)" from the other, depending on how the audit was submitted.
//
...
//   npx dotenv -e .env.local -- npx tsx scripts/audit-ai/_arm-zero-attestation-vercel.ts
import dotenv from "dotenv";
dotenv.config({ path: ".env.local", quiet: true });
const TOKEN = process.env.VERCEL_TOKEN!;
const PROJ = "prj_oqyqfwO0qJmkSAO9Hvt7VxbLUToD";
const TEAM = "team_4FAowTLgslDBY6aZ0acPaES0";
const KEY = "AUDIT_ZERO_ATTESTATION_INCOMPLETE";
