...
console.log("\n── whose document this is ──");
{
  ...
  // DESIGN REFUSED THE STAMP ON THIS CHECK, and they were right about what it measures.
  // It greps the SOURCE for one exact phrase. Two ways that fails: any other wording of the same
  // credit ("Generated by", "Built with") sails through, and the naive strengthening — grep the
  // rendered document for "FARaudit" — is useless here because the customer IS named FARaudit Inc.
  // A gate keyed on the company's own name can neither pass honestly nor fail meaningfully.
  //
  // What the rule actually forbids is OUR credit on THEIR document, so the check is the credit
  // SHAPE — a first-person attribution verb bound to our name — not the name alone. That fires on
  // any phrasing and stays silent when the customer's own letterhead says FARaudit Inc.
  const CREDIT_SHAPE = /\b(prepared|generated|created|built|produced|powered|made)\s+(with|by|using|on)\s+FARaudit\b/i;
  check("the pasted copy carries no FARaudit credit", !CREDIT_SHAPE.test(live),
  ...
  check("the printed document carries no FARaudit credit", !CREDIT_SHAPE.test(pdf),
    "our marketing is on a document the customer sends under their own name");
  // FALSIFIABILITY, in both directions — the reason the old check went unchallenged is that
  // nobody ever asked whether it could fire.
  check("P· the credit check fires on a rewording the old one missed",
    CREDIT_SHAPE.test("<p>Generated by FARaudit</p>"),
    "only the one phrase was ever caught");
  check("P· it does NOT fire on the customer's own name",
    !CREDIT_SHAPE.test("<h1>FARaudit Inc.</h1><p>NAICS 332710</p>"),
    "a customer named FARaudit could never pass a gate keyed on the bare name");
...
// ── a value is present when it says something ────────────────────────────────
// Design reported a trailing comma in the pasted copy on the live record. It did not reproduce —
// contact_title is null and the paste renders the bare name. What the reproduction attempt DID
// find is the adjacent trap: the presence test accepted the literal strings "null" and
// "undefined", so a serialization accident anywhere upstream would print "Jose Rodriguez, null"
// on a document a contracting officer reads. Indistinguishable, at that point, from a claim.
console.log("\n── the presence test rejects serialization accidents ──");
{
  check("the presence test names the accidents it rejects",
    /t !== 'null' && t !== 'undefined'/.test(live),
    "a string 'null' reaches the customer's document as if it were a title");
  // The comma construction must still be guarded in BOTH paste containers, which is the defect
  // Design was pointing at even though the instance did not reproduce.
  const commaGuards = (live.match(/has\(REC\.contact_title\) \? ', ' \+ REC\.contact_title : ''/g) || []).length;
