...
export default async function DashboardPage() {
  ...
  const [activeRes, highScoreRes, totalRes, recentRes] = await Promise.all([
    ...
    supabase
      .from("audits")
      .select(
        "id, notice_id, title, agency, response_deadline, compliance_score, recommendation, status, created_at"
      )
      .eq("user_id", user.id)
    ...
    <div className="min-h-screen">
      ...
      <main className="px-6 md:px-10 py-12 md:py-16 max-w-7xl mx-auto">
        ...
        <p className="mt-3 text-text-2">
          Live solicitations · scores · capture intelligence
        </p>
        ...
        <div className="mt-20">
          <div className="flex items-end justify-between mb-8">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.3em] text-text-3 mb-2">
                Live Feed
              </p>
          ...
          {recent.length === 0 ? (
            <div className="border border-border bg-surface p-16 text-center">
              ...
              <p className="text-sm text-text-3 mt-2 mb-6">
                Audit your first solicitation to see findings here.
              </p>
              <Link
                href="/audit"
...
function Metric({
  ...
  return (
    <div className="bg-surface px-8 py-10">
      <p className="text-xs uppercase tracking-[0.2em] text-text-3 font-mono">
        {label}
      </p>
      <p
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3">
          ...
          {isPending && (
            <span className="font-mono text-xs text-amber uppercase">processing</span>
          )}
          {isFailed && (
            <span className="font-mono text-xs text-red uppercase">failed</span>
          )}
        </div>
        <p className="mt-1.5 text-text truncate font-display text-lg">
      ...
      <div className="flex items-center gap-4">
        {audit.recommendation && (
          <span className="hidden sm:inline-block font-mono text-xs uppercase tracking-[0.2em] text-text-3 group-hover:text-text-2">
            {audit.recommendation === "PROCEED_WITH_CAUTION"
              ? "Caution"
              : audit.recommendation}
          </span>
        )}
