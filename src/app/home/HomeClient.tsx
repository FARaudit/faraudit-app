...
import AppSidebar, { type SidebarItem, type SidebarSection } from "@/components/AppSidebar";
import type {
  HeaderCounter,
  OpportunityRow,
  AuditRow,
  ...
  return (
    <div className="bd-home">
      <div className="app">
        ...
        {/* SIDEBAR — unified <AppSidebar> shared with non-/home routes (Navigation.tsx).
             Caller wires onClick to setTab() so workspace tabs flip in-page state.
             showWordmark=false because /home's topbar already renders FAR<audit>. */}
        <HomeSidebar
          tab={tab}
          setTab={setTab}
...
// /home-side wrapper around <AppSidebar>. Builds the sections from /home's
// in-memory state (counts, current tab) and dispatches each item to
// setTab() so clicks flip the workspace pane rather than navigating routes.
// showWordmark=false because /home's topbar already renders the wordmark.
function HomeSidebar({
  ...
  const workspace: SidebarItem[] = [
    { id: "today", label: "Today", icon: "today", onClick: () => setTab("home"), isActive: tab === "home", badge: null },
    { id: "audit", label: "Run Audit", icon: "audit", onClick: () => setTab("audit"), isActive: tab === "audit", badge: { text: "New", variant: "gold" } },
    { id: "past-audits", label: "Past Audits", icon: "past-audits", onClick: () => setTab("past-audits"), isActive: tab === "past-audits", badge: { text: String(recentAuditsCount), variant: "gold" } },
    { id: "pipeline", label: "Pipeline", icon: "pipeline", onClick: () => setTab("pipeline"), isActive: tab === "pipeline", badge: p0Count > 0 ? { text: String(p0Count), variant: "red" } : null },
    { id: "capability", label: "Capability Statement", icon: "capability", onClick: () => setTab("capability"), isActive: tab === "capability", badge: null }
  ];

  const intelligence: SidebarItem[] = [
    { id: "opportunities", label: "Opportunities", icon: "opportunities", onClick: () => setTab("opportunities"), isActive: tab === "opportunities", badge: { text: "Live", variant: "green" } },
    { id: "defense-spending", label: "Defense Spending", icon: "defense-spending", onClick: () => setTab("defense-spending"), isActive: tab === "defense-spending", badge: null },
    { id: "news", label: "Defense News", icon: "news", onClick: () => setTab("news"), isActive: tab === "news", badge: null },
    { id: "contracting-officers", label: "Contracting Officers", icon: "contracting-officers", onClick: () => setTab("contracting-officers"), isActive: tab === "contracting-officers", badge: kosCount > 0 ? { text: String(kosCount), variant: "gold" } : null },
    { id: "agencies", label: "Agencies", icon: "agencies", onClick: () => setTab("agencies"), isActive: tab === "agencies", badge: agenciesCount > 0 ? { text: String(agenciesCount), variant: "gold" } : null },
    { id: "protests", label: "GAO Protests", icon: "protests", onClick: () => setTab("protests"), isActive: tab === "protests", badge: null },
    { id: "regulatory", label: "FAR/DFARS Updates", icon: "regulatory", onClick: () => setTab("regulatory"), isActive: tab === "regulatory", badge: null },
    { id: "cmmc", label: "CMMC Readiness", icon: "cmmc", onClick: () => setTab("cmmc"), isActive: tab === "cmmc", badge: null },
    { id: "wages", label: "Wage Benchmarks", icon: "wages", onClick: () => setTab("wages"), isActive: tab === "wages", badge: null },
    { id: "teaming", label: "Teaming Partners", icon: "teaming", onClick: () => setTab("teaming"), isActive: tab === "teaming", badge: null }
  ];

  const account: SidebarItem[] = [
    { id: "settings", label: "Profile & Settings", icon: "settings", onClick: onProfileClick, isActive: false, badge: null },
    { id: "signout", label: signingOut ? "Signing out…" : "Sign Out", icon: "signout", onClick: () => { if (!signingOut) void signOut(); }, isActive: false, badge: null }
  ];
  ...
  return <AppSidebar sections={sections} mode="inflow" showWordmark={false} showUpgradeCTA={true} />;
}
