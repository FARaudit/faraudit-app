...
const R10_THIRD_PARTY_STATUS: RegExp[] = [
  ...
  /\b(?:cost\s+accounting\s+practices?|CAS\s+disclosure\s+statement|disclosure\s+statement|billing\s+rates?|forward\s+pricing\s+rates?)\b[^.\n]{0,60}\b(?:dis|in|un|non[- ]?)?(?:approv\w+|accept\w+|adequa\w+|validat\w+|audit\w*|review\w*|surve\w+|compl\w+|meets?\b|satisf\w+|determin\w+|deficien\w+|material\s+weakness\w*|withh?[eo]ld\w*)\b/i,
