...
const R10_THIRD_PARTY_STATUS: RegExp[] = [
  ...
  /\b(?:DCAA[- ]\w+|DCMA[- ]\w+)\b[^.\n]{0,45}\b(?:accounting|billing|purchasing|estimating|property(?:\s+management)?|material\s+management(?:\s+and\s+accounting)?|business|earned[- ]value|EVMS?|MMAS)\s+systems?\b/i,
  /\b(?:DCAA|DCMA|CFA|Defense\s+Contract\s+(?:Audit|Management)\s+Agency|[Cc]ognizant\s+[Ff]ederal\s+[Aa]gency|cognizant\s+(?:\w+\s+){0,2}(?:agency|auditor|activity|ACO))\b[^.\n]{0,80}\bsystems?\b/i,
  /\bsystems?\b[^.\n]{0,80}\b(?:DCAA|DCMA|CFA|Defense\s+Contract\s+(?:Audit|Management)\s+Agency|[Cc]ognizant\s+[Ff]ederal\s+[Aa]gency|cognizant\s+(?:\w+\s+){0,2}(?:agency|auditor|activity|ACO))\b/i,
