...
const R10_THIRD_PARTY_STATUS: RegExp[] = [
  ...
  /\b(?:sources?|manufacturers?|suppliers?|vendors?|dealers?)\s+(?:approved|qualified)\s+by\b|\bsource[- ]approv\w+\b|\bsource\s+approval\b|\bsource[- ]control(?:led)?\b|\bsource\s+control\s+(?:drawing|document)s?\b|\bqualifying\s+activity\b|\b(?:subject\s+to|satisf\w+|met)\s+(?:a\s+|all\s+(?:applicable\s+)?)?qualification\s+requirements?\b|\bstandards\s+(?:prescribed|established)\s+for\s+qualification\b|\bqualification\s+requirements?\b[^.\n]{0,140}\b(?:before|prior\s+to)\b[^.\n]{0,15}\baward\b/i,
