...
5. Each confidence note (CONDITION 1 fail-loud) is also a STRUCTURED OBJECT:
     ...
     resolve    — one sentence stating HOW to confirm (e.g. ${isEnvOn(process.env.AUDIT_FORM_KEYED_CITATION) ? `"Verify against the solicitation cover form / SAM notice"` : `"Verify against SF-1449 block 10"`})
...
// [V1/shadow purged 2026-06-28 — A4] runJudgment() removed — engine is 100% agentic (executeAgenticPrimary → auditPackage). See git history.
