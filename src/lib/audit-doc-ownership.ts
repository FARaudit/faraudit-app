// ── DOCUMENT OWNERSHIP MAP — deterministic, $0, name-based. THE SINGLE COPY. ─────────────────────────
//
// Moved here from `scripts/audit-ai/_ownership-map-proposal.ts` on 2026-08-19 (Brain rulings R1/R6).
// That path now RE-EXPORTS this module and holds no rules of its own: a rule table living in two files
// is two rules, and the one under scripts/ could not be imported by production code at all.
//
// DOCTRINE (recorded in ENGINE-OWNERSHIP-MAP-2026-08-17.md, Brain ruling CEO-approved 2026-08-17):
//   1 OWNERSHIP IS TOTAL — every obligation-carrying document gets exactly one owner before any lens
//     fires; an unassignable document is a NAMED coverage gap, never a silence.
//   2 OWNERSHIP IS DOCUMENT-KEYED, NOT SECTION-KEYED — sections are a routing hint; the document is
//     the unit. Four of the five live lanes are keyed to UCF sections, which is why 48 of 49
//     obligation-carrying documents on the flagship are structurally unownable today.
//   3 ROUTING IS 1:1 — one document, exactly one owner, NO fan-out. Fanning the mandate across five
//     lenses is what blew the 270s budget on live runs 6cbabeae / e63a9b2d.
//
// Allowlist shapes ONLY: an unmatched name falls to RESIDUE and is NAMED, never silently defaulted.
// Built from the 135 binding-document names actually observed across the banked corpus, not from
// imagined categories.
