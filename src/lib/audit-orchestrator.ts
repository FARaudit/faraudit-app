...
import { SITE_VISIT_CONCLUDED_RE, BOA_IDIQ_HOLDER_BAR_RE } from "./audit-site-visit-patterns";
...
export function emitNoticeBodyEligBarFindings(fullSource: string, findings: TypedFinding[], noticeBodyText?: string | null): TypedFinding[] {
  ...
  // SINGLE-VISIT guard for the notice-wide concluded fallback: count DISTINCT site-visit attendance-bar sentences.
  // The notice-wide fallback (below) may only attribute a lone concluded marker to a bar when the notice describes
  // exactly ONE site visit — a multi-visit notice (one concluded + one upcoming) must keep the window-scoped search,
  // or the fallback would mis-frame the upcoming visit as concluded (regression pair test #9).
  ...
  for (const m of nNotice.matchAll(new RegExp(ELIGIBILITY_BAR_RE.source, "gi"))) {
    ...
    if (NOTICE_SITE_VISIT_RE.test(nNotice.slice(s, e))) svBarSpans.add(`${s}:${e}`);
  }
  const singleSiteVisit = svBarSpans.size === 1;
  const scan = new RegExp(ELIGIBILITY_BAR_RE.source, "gi");
  for (const m of nNotice.matchAll(scan)) {
  ...
  if (process.env.AUDIT_NOTICE_BODY_BOA_EMIT === "true") {
    const bscan = new RegExp(BOA_IDIQ_HOLDER_BAR_RE.source, "gi");
