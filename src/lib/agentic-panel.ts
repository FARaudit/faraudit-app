...
// ROOT-2 (flag AUDIT_SITEVISIT_ELIGIBILITY, arc #1 2026-07-10): a MANDATORY pre-proposal
// site visit / pre-bid conference whose attendance is a stated CONDITION of eligibility is
// temporal — COMPLETED + non-attendee = uncurable → DISQUALIFYING; UPCOMING = gate-to-clear.
// The engine previously typed it as a bidder-controlled gate-to-clear (or missed it), so a
// disqualified non-attendee could be told a barred contract was winnable (the catastrophic
// false-NO-BID-miss the CBP /panel caught). Appended to DISPOSITION so BOTH the lenses and the
// synthesizer apply it. Flag-OFF ⇒ empty string ⇒ DISPOSITION byte-identical. Read at module
// load (worker sets env before boot, mirroring the other agentic flags).
const SITE_VISIT_ELIGIBILITY = isEnvOn(process.env.AUDIT_SITEVISIT_ELIGIBILITY)
  ? " TEMPORAL ELIGIBILITY — MANDATORY SITE VISIT / PRE-PROPOSAL CONFERENCE: when the solicitation makes ATTENDANCE at a pre-proposal site visit or pre-bid conference a stated CONDITION of eligibility to propose (e.g. 'only firms that attended may submit / move forward with the proposal process'), it IS a real eligibility gate — raise it as a named_hard_gate (met=false), never omit it. CRITICAL: the audit does NOT know whether THIS bidder attended (attendance is an out-of-band fact the engine does not have), so you MUST NEVER emit an unconditional NO_BID / INELIGIBLE on it. Instead: (a) if the visit has ALREADY OCCURRED / is COMPLETED (a non-attendee cannot make it up), surface it as a CONDITIONAL DISQUALIFYING bar and route the verdict to NEEDS_HUMAN_REVIEW, stating the conditional explicitly — 'NO-BID IF your firm did not attend the completed mandatory site visit; confirm attendance' — met=false, never a silent pass, never an unconditional NO_BID; (b) if it is still UPCOMING, it is a bidder-CONTROLLED gate-to-clear (the firm attends it) — enumerate it, never a downgrade; (c) if it is UNCLEAR whether the visit is completed or upcoming (no anchorable date), treat it as (a) — conditional bar + NEEDS_HUMAN_REVIEW, never a committal either way. A site visit that is OPTIONAL / merely offered / recommended, or a PERFORMANCE-PHASE / post-award / contract-administration site visit, is NOT an eligibility gate at all — do not raise it as a hard gate."
  : "";
// EMISSION-SPLIT (Brain card #609 third bundled fix, flag AUDIT_EMISSION_SPLIT default-OFF). ROOT: a lens that bundles a
// compound "maintain licensing, certification, accreditation AND insurance … proof at award" sentence into ONE gate forces
// the downstream typing floor to fail-closed on the whole bundle (a possession frame or one scarce constituent taints all
// of it) → false NHR. The gold-set (40fd02ce) emitted the SAME obligations as SEPARATE, correctly-typed findings → BWC.
// This instructs one-obligation-per-finding, each with its OWN full verbatim excerpt + own typing. ANTI-DILUTION (binding
// red-team): the split may NEVER weaken a bar — a scarce/long-lead constituent emits as its own gate and STILL escalates;
// a governing possession/at-award frame attaches to EVERY constituent it governs, not just the nearest noun; a joint scarce
// context keeps the scarce read after the split. Flag-OFF ⇒ empty string ⇒ DISPOSITION byte-identical.
const EMISSION_SPLIT = isEnvOn(process.env.AUDIT_EMISSION_SPLIT)
  ? " EMISSION GRANULARITY — ONE OBLIGATION PER GATE: when a single source sentence bundles MULTIPLE distinct obligations (e.g. 'maintain licensing, certification, accreditation and insurance of $1M/$3M; proof at time of award'), emit each obligation as its OWN separate named_hard_gate — do NOT collapse them into one compound gate. Each split gate MUST carry (a) the COMPLETE verbatim source sentence as its excerpt (the full sentence that governs it — never a truncated fragment; the same sentence may be shared verbatim across the constituents it governs), and (b) its OWN controllability typing under the DISPOSITION rule (an ordinary self-acquirable credential — business/professional licensing, an insurance policy incl. proof-at-award, SAM registration, size self-cert — is a bidder-controlled GATE-TO-CLEAR, met=false; each typed on its own merits). ANTI-DILUTION (never let a split soften a real bar): if ANY constituent is a SCARCE / LONG-LEAD credential the firm cannot self-acquire in the response window — facility or personnel security clearance, CMMC / NIST 800-171 at award, a surety bond, ITAR/EAR export authorization, a QPL/QML listing, or a scarce accreditation that functions as a gate — emit THAT constituent as its own DISQUALIFYING gate (met=false, bidder-uncontrollable) and keep it escalating regardless of the ordinary constituents beside it. A governing possession / at-award frame ('proof at time of award', 'prior to award', 'as a condition of award', 'must currently hold') attaches to EVERY constituent that sentence grammatically governs, not only the noun nearest to it. When a phrase names a scarce credential inside an otherwise-ordinary list ('licensing including facility clearance'), the SCARCE reading survives the split — never downgrade it to routine licensing."
  : "";
const DISPOSITION = "DISPOSITION + CONTROLLABILITY RULE (pre-bid — no proposal exists yet): classify EVERY requirement by who CONTROLS its satisfaction. (1) BIDDER CONTROLS it — sourcing, pricing, configuration, documentation, certs / brochures / samples to provide, prices to submit → an UNMET GATE-TO-CLEAR (met=false): the bidder resolves it by doing the work. It does NOT disqualify and does NOT downgrade the verdict — NEVER mark such a requirement as a disqualifying / show-stopper gate. (2) BIDDER CANNOT move it regardless of effort — a failed eligibility bar, a single-source / proprietary spec the firm cannot legally supply, unattainable past-performance, exclusivity → a DISQUALIFYING bar. Set met=true ONLY for facts already structurally true of the SOLICITATION or THIS bidder right now (set-aside type, the firm's existing SAM registration / size status, a deadline that has or hasn't passed). NARROW ≠ disqualifying: a narrow but COMMODITY spec (a machine class multiple OEMs make) is a sourcing task the bidder controls → gate-to-clear, never disqualifying. A provision the firm is ELIGIBLE for (e.g. the set-aside it qualifies under) is met=TRUE (a structural fact ALREADY satisfied), NOT an unmet gate. Do NOT put standard FAR BOILERPLATE in named_hard_gates AT ALL — EEO / Anti-Discrimination / Diversity-Equity-Inclusion clauses, standard commercial terms & conditions, and unenforceable-supplier-terms boilerplate are routine compliance, NOT gates; omit them from named_hard_gates entirely (mention in risks at most). A named_hard_gate is a REAL pass/fail requirement, never boilerplate." + SITE_VISIT_ELIGIBILITY + EMISSION_SPLIT;
