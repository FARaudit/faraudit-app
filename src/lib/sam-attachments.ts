...
export function planDocumentOrder(entries: AttachmentManifestEntry[], solicitationNumber: string | null): DocumentPlanEntry[] {
  ...
  const rank = { form: 0, amendment: 1, attachment: 2 } as const;
  return planned.sort((a, b) => rank[a.role] - rank[b.role] || a.name.localeCompare(b.name));
}
...
export function applyBudget(plan: DocumentPlanEntry[], maxDocs = MAX_DOCS, maxTotal = MAX_TOTAL_INLINE_BYTES): {
  ...
  let total = 0;
  // Fill order: form + amendments in plan order (always first — they ARE the
  // solicitation), then attachments SIZE-ASCENDING. Small attachments are
  // text-dense, high-audit-value documents (wage determinations, RFI logs,
  // specifications); the huge members are drawing sets and survey scans with
  // low text value per MB. Deterministic either way — and everything skipped
  // is named in the overflow flag.
  const fillOrder = [
    ...plan.filter((e) => e.role !== "attachment"),
    ...plan.filter((e) => e.role === "attachment").sort((a, b) => (a.sizeBytes ?? Infinity) - (b.sizeBytes ?? Infinity) || a.name.localeCompare(b.name))
  ];
